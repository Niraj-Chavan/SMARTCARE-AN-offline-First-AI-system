  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-72 rounded-xl border border-slate-200 bg-white/95 p-4 text-xs text-slate-700 shadow-lg backdrop-blur">
      ...
      <button
        type="button"
        onClick={() => {
          localStorage.removeItem('smartcare_onboarded')
          window.location.reload()
        }}
        className="mt-4 w-full rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100"
      >
        Reset onboarding
      </button>
    </div>
